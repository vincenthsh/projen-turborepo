...
export class TurborepoProject extends typescript.TypeScriptProject {
  ...
  constructor(options: TurborepoProjectOptions) {
     ...
     */
    this.npmrc.addRegistry("https://npm.pkg.github.com/", "@vincenthsh");
    this.addDevDeps("@vincenthsh/projen-turborepo");

    // give workflows permissions to read packages
    const buildWorkflowYaml = this.tryFindObjectFile(
  ...
  preSynthesize() {
            ...
            for (const [packageName, packagePath] of Object.entries(
              ...
              if (baseUrl === ".") {
                tsconfig?.file.addOverride(
                  `compilerOptions.paths.${packageName}`,
                  [packagePath],
                );
              } else {
    ...
    if (this.vscodeMultiRootWorkspaces) {
      ...
      if (this.eslint) {
        vscodeConfig.settings["eslint.workingDirectories"] = subProjects.map(
          (subProject) => `./${path.relative(this.outdir, subProject.outdir)}`,
        );
      }
